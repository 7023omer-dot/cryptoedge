...
5. **כשמשתמש שולח תמונה — חלץ אותה בעצמך, אל תבקש העלאה ידנית.** התמונה שמורה כ-base64 בקובץ השיחה (`/root/.claude/projects/.../<session>.jsonl`, שדה `{"type":"image","source":{"type":"base64",...}}`). סקריפט פייתון: לפרסר כל שורת JSON, ללכת רקורסיבית, לדקד base64, לשמור כקובץ (webp/jpg/png) בתוך הפרויקט. webp נתמך בכל הדפדפנים. **שים לב:** `vercel.json` שמנתב הכל ל-index.html חוסם קבצים סטטיים — צריך regex שמחריג סיומות (webp/png/jpg/js/css וכו').
